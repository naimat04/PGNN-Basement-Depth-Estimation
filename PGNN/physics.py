...
def calculate_gravity_field(depth_map, r0=DEFAULT_R0, lambda_=DEFAULT_LAMBDA,
    ...
    def np_gravity_fn(depth_map_np):
        batch_size, ny, nx, _ = depth_map_np.shape
        gravity_output = []
        for b in range(batch_size):
            z_sample = depth_map_np[b, :, :, 0]  # (ny, nx)
            g = FW_Granser(z_sample * 10, r0, lambda_, nx, ny, dx, dy, n, const)
            gravity_output.append(g)
        gravity_output = np.stack(gravity_output, axis=0)  # (batch, ny, nx)
        return gravity_output.astype(np.float32)

    gravity_field = tf.numpy_function(np_gravity_fn, [depth_map], tf.float32)
    # Fix shape: (batch, ny, nx, 1)
    gravity_field = tf.expand_dims(gravity_field, -1)
    gravity_field.set_shape(depth_map.shape)  # ensure static shape info
